...
the latest version that has matching packages is selected.`,
  ...
  chef-pkg download packages --product chef --version 18.4.12 --channel stable

  # Download the latest version
  chef-pkg download packages --product chef --version latest
  ...
  chef-pkg download packages --product chef --version 18.4.12 --channel stable --platform ubuntu

  # Download to a custom directory with higher concurrency
  chef-pkg download packages --product chef --version 18.4.12 --dest /tmp/chef-packages --concurrency 8`,
...
func init() {
	...
	downloadPackagesCmd.Flags().StringP("product", "p", "chef", "Chef product name (e.g. chef, chef-ice, inspec)")
	downloadPackagesCmd.Flags().StringP("version", "v", "latest", "Product version: semver (e.g. 18.4.12), 'latest', or 'all' (default: latest)")
	downloadPackagesCmd.Flags().String("platform", "", "Filter by platform (substring match, case-insensitive)")
	downloadPackagesCmd.Flags().String("arch", "", "Filter by architecture (substring match, case-insensitive)")
	downloadPackagesCmd.Flags().StringP("dest", "d", "", "Destination root directory (default: ./packages)")
	downloadPackagesCmd.Flags().Bool("skip-existing", true, "Skip files that already exist with matching SHA256")
	downloadPackagesCmd.Flags().IntP("concurrency", "c", 0, "Max parallel downloads (default: 4)")

	viper.BindPFlag("download.dest", downloadPackagesCmd.Flags().Lookup("dest"))
	viper.BindPFlag("download.concurrency", downloadPackagesCmd.Flags().Lookup("concurrency"))
}
