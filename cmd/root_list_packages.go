...
the latest version that has matching packages is selected.`,
  ...
  chef-pkg list packages --product chef-ice --version 19.1.158

  # List packages for the latest version
  chef-pkg list packages --product chef --version latest
  ...
  chef-pkg list packages --product chef --version all --arch aarch64

  # Filter by platform
  chef-pkg list packages --product chef-ice --version 19.1.158 --platform ubuntu

  # Output as JSON
  chef-pkg list packages --version 19.1.158 --output json`,
...
func init() {
	...
	listPackagesCmd.Flags().StringP("product", "p", "chef", "Chef product name (e.g. chef, chef-ice, inspec)")
	listPackagesCmd.Flags().StringP("version", "v", "latest", "Product version: semver (e.g. 18.4.12), 'latest', or 'all' (default: latest)")
	listPackagesCmd.Flags().String("platform", "", "Filter results by platform (substring match, case-insensitive)")
	listPackagesCmd.Flags().String("arch", "", "Filter results by architecture (substring match, case-insensitive)")
	listPackagesCmd.Flags().StringP("output", "o", "table", "Output format: table or json")
}
